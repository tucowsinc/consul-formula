...
vault-write-agent-token:
  module.run:
    - vault.write_secret:
      - path: 'kv/data/tenants/{{ tenant_name }}/bootstrap/consul/consul_agent_token'
      - data:
...
vault-write-anon-token:
  module.run:
    - vault.write_secret:
      - path: 'kv/data/tenants/{{ tenant_name }}/bootstrap/consul/consul_anon_token'
      - data:
...
vault-write-vault-token:
  module.run:
    - vault.write_secret:
      - path: 'kv/data/tenants/{{ tenant_name }}/bootstrap/consul/consul_vault_token'
      - data:
          id: {{ vault_content_vault }}
{% endif %}

{%- if salt['vault.read_secret']('kv/data/tenants/'~ tenant_name ~'/bootstrap/consul/consul_vault_token') %}
set-bootstrap-grain:
  grains.present:
    - name: vaultwritten
    - value: True
